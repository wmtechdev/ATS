...
typedef DataRowBuilder<T> =
    DataRow? Function(T, int, bool, void Function(bool?)?);

class FlutterLibDataTableController<T> extends DataTableSource {
  FlutterLibDataTableController({
    List<T>? initialData,
    int? numRows,
...
class FlutterLibDataTable<T> extends StatefulWidget {
  const FlutterLibDataTable({
    super.key,
  ...
  final FlutterLibDataTableController<T> controller;
  final List<T> data;
  final int? numRows;
  ...
  @override
  State<FlutterLibDataTable<T>> createState() => _FlutterLibDataTableState<T>();
}

class _FlutterLibDataTableState<T> extends State<FlutterLibDataTable<T>> {
  FlutterLibDataTableController<T> get controller => widget.controller;
  ...
  @override
  void didUpdateWidget(FlutterLibDataTable<T> oldWidget) {
  ...
  Widget build(BuildContext context) {
    final columns = widget.columnsBuilder((index, ascending) {
        ...
        child: SizedBox(
          ...
          child: Theme(
            ...
            child: PaginatedDataTable2(
              ...
              columns: columns,
              empty: widget.emptyBuilder != null
                  ? widget.emptyBuilder!()
                  : null,
              ...
              dividerThickness: widget.hideDefaultHorizontalDivider
                  ? 0.0
                  : null,
  ...
  double _getColumnsWidth(List<DataColumn> columns) => columns
      .where((c) => c is DataColumn2 && c.fixedWidth != null)
      .fold(
